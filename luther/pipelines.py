...
class LutherPipeline:

    # Database setup
    conn = sqlite3.connect('luther.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `luther`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM luther WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO luther (date, title, link, content)"
